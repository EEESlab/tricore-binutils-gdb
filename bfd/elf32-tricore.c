...
// TODO int tricore_elf32_pcpmap = -1;

/* True if PCP address mappings should be printed (for debugging only).  */

// TODO bool tricore_elf32_debug_pcpmap = false;
...
tricore_elf32_add_exported_symbol
   (const char *name, bool type, bfd_size_type size, const char *export_name)
{
#if 0
TOCHECK	
...
void
tricore_elf32_do_export_symbols(struct bfd_link_info *info)
{
#if 0
TOCHECK	
...
static bool
tricore_elf32_relax_section (bfd *abfd,
     asection *sec,
     struct bfd_link_info *info,
     bool *again)
{
  ...
  if (!initialized)
    {
      ...
      ++max;
      if (((rela_malloced = bfd_malloc (max * sizeof (bool))) == NULL)
  ...
  for (irel = internal_relocs; irel < irelend; irel++)
	...
	continue;
	  ...
	  struct bfd_link_hash_entry *h2;
          ...
          h2 = bfd_link_hash_lookup (info->hash, "CPUBUG14",
	  			    false, false, false);
          if ((h2 != (struct bfd_link_hash_entry *) NULL)
	      && (h2->type == bfd_link_hash_defined)
	      && (h2->u.def.value != 0))
            add_bytes = 20;
	  else
	    add_bytes = 12;
